...
- **Open / next:** push commit; then step 4 — loop wiring. Compute bias parameters from the prior trajectory via `sampling/bias_designer.py` (σ ≈ width of the observable along the proposed CV; height ≈ k_B T; pace ≈ rule-of-thumb), write `plumed.dat`, reconfigure the adapter with `plumed_input`, start the next round biased. Step 5 (chignolin metaD live smoke) follows.
